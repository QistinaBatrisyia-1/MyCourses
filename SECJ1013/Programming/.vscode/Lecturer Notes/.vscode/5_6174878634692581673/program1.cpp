...
int displayMenu(int& option) {//Error 5 -Syntax Error
    cout << "----------------------------------\n";
    ...
    while (option < 1 || option > 3) {//Error 4 Logic Error
        cout << "Invalid option! Enter valid option (1,2,3) : ";
        cin >> option;
    }
    return option;//Error 1-Syntax Error
}
...
float calcRentCar(int carModel, float hourRate) {//Error 2 -- Syntax Error
...
int main() {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            cin >> hour;
            float rent = calcRentCar(car,hour);//syntax Error 9
